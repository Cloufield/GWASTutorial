...
!!! note "Unrelated individuals"
    For PLINK association tests, we include only unrelated individuals. This is because standard linear/logistic regression does not account for relatedness, and including related individuals can lead to inflated test statistics and false positives. Kinship-pruned samples (unrelated individuals) should be used for association testing. For datasets with related individuals, consider using [Linear Mixed Models (LMM)](https://cloufield.github.io/GWASTutorial/33_linear_mixed_model/), which can properly account for genetic relatedness.

!!! note "Firth correction"
    Adding a penalty term to the log-likelihood function when fitting the logistic model results in less bias. 
    ...
    where $L(\beta)$ is the standard log-likelihood function and $I(\beta)$ is the Fisher information matrix.
    
    Reference: Firth, David. "Bias reduction of maximum likelihood estimates." Biometrika 80.1 (1993): 27-38.
...
!!! info "Usually, other options are added to enhance the sumstats"
    ...
    * --covar-variance-standardize	# To normalize the covariates which may be at a huge scale, like AGE**AGE.
...
Then, we can use the genomic control factor to correct observed Chi square statistics.
...
In genome-wide association studies, we test millions of genetic variants for association with a trait. This creates a multiple testing problem: when performing many statistical tests, the probability of observing at least one false positive increases with the number of tests performed.

To control the family-wise error rate (FWER), we need to adjust the significance threshold. The most conservative approach is the **Bonferroni correction**, which divides the nominal significance level (typically α = 0.05) by the number of independent tests performed.
...
!!! info "Why 5 × 10⁻⁸?"
    ...
    The genome-wide significance threshold of **P < 5 × 10⁻⁸** (or 5e-8) is widely used in GWAS. This threshold is based on:
    
    1. **Bonferroni correction**: Starting with α = 0.05 and dividing by approximately 1,000,000 independent tests:
       $$
       \alpha_{adjusted} = \frac{0.05}{1,000,000} = 5 \times 10^{-8}
       $$
    
    2. **Independent tests**: Although modern GWAS can test millions of variants, not all are independent due to linkage disequilibrium (LD). The number of independent tests is estimated to be around 1 million based on the effective number of independent variants in the human genome. This estimate was originally derived from analyses of the HapMap3 reference panel, which characterized patterns of LD across diverse populations.
    
    3. **Historical convention**: This threshold was established in early GWAS studies and has become the standard in the field.

!!! note "Multiple testing correction methods"
    
    While Bonferroni correction is conservative and widely used, other methods exist:
    ...
    - **Permutation-based methods**: Empirically determine the significance threshold by permuting phenotypes.
    - **Effective number of tests**: Adjusts for LD structure to estimate the true number of independent tests (e.g., using spectral decomposition of the LD matrix).
    
    However, **P < 5 × 10⁻⁸** remains the gold standard for declaring genome-wide significance in GWAS.

!!! warning "Suggestive significance"
    
    Variants with P values between 1 × 10⁻⁵ and 5 × 10⁻⁸ are sometimes considered "suggestive" associations. These may warrant replication in independent cohorts but should not be considered definitively associated without further validation.
...
Data we need from sumstats to create the Q-Q plot:
...
!!! tips "Steps to create Q-Q plot"
    ...
    2. sort the -log10(P) values in ascending order.
