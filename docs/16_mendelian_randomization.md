...
!!! warning "Triangulation"
    Before running MR analyses, it is important to understand that MR alone is rarely sufficient for robust causal inference. MR has inherent assumptions and potential biases (pleiotropy, population stratification, weak instruments) that can affect results. Therefore, plan to complement your MR findings with other epidemiological approaches (e.g., randomized controlled trials, observational studies, negative controls) that have different bias structures. When results converge across methods, confidence in causality is strengthened; when they diverge, this helps identify which biases may be affecting your analysis.
...
Visualize the association between genetic variants and exposure (x-axis) versus outcome (y-axis) to assess the MR relationship.
...
Forest plot showing the individual causal effect estimate from each instrumental variable.
...
Plot of effect estimates against their precision to visually assess potential publication bias or heterogeneity.
