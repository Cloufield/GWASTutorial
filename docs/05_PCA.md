...
In the projection, `score ${outPrefix}.eigenvec.allele 2 6` sets the `ID` (2nd column) and `A1` (6th column), `score-col-nums 7-16` sets the first 10 PCs to be projected. Please check https://www.cog-genomics.org/plink/2.0/score#pca_project for more details on the projection.
...
!!! info "What is UMAP?"
    **UMAP (Uniform Manifold Approximation and Projection)** is a non-linear dimension reduction technique that preserves both local and global structure in high-dimensional data. Unlike PCA, which finds linear projections, UMAP can capture non-linear relationships and is particularly useful for visualizing complex data structures.

We can also apply UMAP to the PCs to further identify the local structures. (PCA-UMAP)
