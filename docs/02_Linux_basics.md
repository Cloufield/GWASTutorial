...
!!! example "Find files by type and size"
    ```Bash
...
!!! example "Find and execute commands"
    ```Bash
...
Common operations in `less`:
- `Space` or `Page Down`: move forward one page
- `b` or `Page Up`: move backward one page
- `q`: quit
- `/pattern`: search for a pattern (press `n` for next match)
- `G`: go to end of file
- `g`: go to beginning of file

!!! example "View a large file using `less`"
    ```Bash
    $ less sumstats.txt
    ```
    This will open the file in `less` viewer. Use `q` to quit.

!!! tip "Why use `less` instead of `cat`?"
...
!!! example "Combine grep with pipes"
    ```Bash
...
!!! tip "Regular expressions with grep"
...
For more detailed tutorials on Vim, you can check: [Learn-Vim](https://github.com/iggredible/Learn-Vim)
...
- `-d` or `--max-depth`: limit depth of directory traversal
- `-a`: show files as well as directories

!!! example "Check directory sizes"
    ```Bash
...
!!! example "Run processes in background"
    ```Bash
...
!!! tip "Best practices for long analyses"
...
!!! example "Find the location of a command"
    ```Bash
...
!!! example "Calculate and verify checksums"
    ```Bash
...
!!! example "Verify downloaded files"
    ```Bash
